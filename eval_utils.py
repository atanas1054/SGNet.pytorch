...
def plot_trajs(trajs_pred, trajs_obs, first_history_index):

    mat = lambda angle: np.array([[np.cos(angle), np.sin(angle)],
                                  [-np.sin(angle), np.cos(angle)]])

    rot_trajs_obs = np.dot(trajs_obs[:, :, [0,1]], mat(np.deg2rad(0)))
    rot_trajs_pred = np.dot(trajs_pred[:, -1, :, :], mat(np.deg2rad(0)))

    first_history_index_ = first_history_index[0]
    for i in range(trajs_pred.shape[0]):
        if i%1==0:

            #y = trajs_pred[i, -1, :, 0]
            #x = trajs_pred[i, -1, :, 1]
            x = rot_trajs_pred[i,:,0]
            y = rot_trajs_pred[i,:,1]
            plt.plot(x, y, color="grey", alpha=0.5, marker="o", linewidth=0.5, markersize=1)
            x = rot_trajs_obs[i, first_history_index_ :, 0]
            y = rot_trajs_obs[i, first_history_index_ :, 1]
            plt.plot(x, y, color="grey", alpha=0.5, marker="o", linewidth=0.5, markersize=1)
...
def compute_batch_ade_fde(target_traj, cvae_all_traj):
    ...
    return result['ADE_12'], result['FDE_12']
